...
class out:
    ...
    def rcl(rclmsg):
        print(f"{Colors.bright_red}Recent call last:{Colors.reset} {rclmsg}")

    def success(successtext):
        print(f"{Colors.bright_green}success:{Colors.reset} {successtext}")
...
class Interpreter(misc, Evaluator):
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        where = "Runtime"
                    #and raise an error
                    self.skiylia.error(token, token, message, where)
                    out.rcl(f"{e}")
                except Exception as exx:
                    out.error(exx)
    ...
    def ExtensionStmt(self, stmt):
        ...
        if stmt.superclass:
            superclass = self.evaluate(stmt.superclass)
            if not isinstance(superclass, SkiyliaClass):
                raise RuntimeError([stmt.superclass.name, "Superext must be a class."])
        #ensure the class is defined in our environment
        self.environment.define(stmt.name.lexeme, None)
        #and add the definition of "super" in our scope
        if stmt.superclass:
