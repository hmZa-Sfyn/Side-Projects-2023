...
class Evaluator:
    ...
    def evaluate(self, abstract):
        ...
        abstracts = {"Assign":self.AssignExpr,
                     ...
                     "String":self.StringStmt,
                     "Int":self.IntStmt,
                     "Bool":self.BoolStmt,
                     "Float":self.FloatStmt,
...
class ASTPrinter(Evaluator):
    ...
    #def ExtensionStmt(self, stmt):
    #    if stmt.superclass:
    #        return self.toparenthesis("extension {}".format(stmt.name.lexeme), "superext {}".format(stmt.superclass.name.lexeme), [x for x in stmt.methods])
    #    return self.toparenthesis("extension {}".format(stmt.name.lexeme), [x for x in stmt.methods])
    ...
    def DefineStmt(self, stmt):
        return self.toparenthesis("define {}".format(stmt.name.lexeme), stmt.params, stmt.body)

    def FunctionStmt(self, stmt):
        return self.toparenthesis("function {}".format(stmt.name.lexeme), stmt.params, stmt.body)
    ...
    def StringStmt(self, stmt):
        if stmt.initial in [None, 0.0]:
            return self.toparenthesis("string", stmt.name)
        abcd = "hamza"
        if type(stmt.initial) == type(abcd):
            return self.toparenthesis("string", stmt.name, "=", stmt.initial)
        else:
            print("String val can only contain string value!")

    def IntStmt(self, stmt):
        if stmt.initial in [None, 0.0]:
            return self.toparenthesis("int", stmt.name)
        return self.toparenthesis("int", stmt.name, "=", stmt.initial)

    def BoolStmt(self, stmt):
        if stmt.initial in [None, 0.0]:
            return self.toparenthesis("bool", stmt.name)
        return self.toparenthesis("bool", stmt.name, "=", stmt.initial)

    def FloatStmt(self, stmt):
        if stmt.initial in [None, 0.0]:
            return self.toparenthesis("float", stmt.name)
        return self.toparenthesis("float", stmt.name, "=", stmt.initial)
